...
for eigindex=32:60 % eigenvalue index 0,1,2 in descending energy order
	fprintf('\n@@ Eigen index: %d\n',eigindex)

	for L_max=30:30 % range of L_max
		fprintf('\n@@ L_max: %d\n',L_max)
		...
		maxSlepian=max(abs(slepian(:))); % for plot normalization
		...
		outname=sprintf('figs/%s_%02d_%02d',basename,L_max,eigindex);
...
if 1 % render movie on osx; needs convert and avconv via brew install libav
	shellcmd=sprintf('convert -delay 30 -quality 100 figs/%s_%02d_* figs/%s-%02d.mov', ...
